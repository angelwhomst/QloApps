...
class HousekeeperTaskDetailController extends ModuleAdminController
{
    ...
    // Override to make sure ofview access 
    ...
    {
        // handle AJAX reqs for this page
        if (Tools::isSubmit('ajax')) {
            ...
            switch ($action) {
                ...
                case 'markTaskDone':
                    ...
                    // vheck if all steps are completed (either passed or failed)
                    $completion = TaskStepModel::getTaskCompletion($id_task);
                    if ($completion['completed'] < $completion['total']) {
                        $response['message'] = 'Cannot mark task as done because there are incomplete checklist items';
                        break;
                    }
