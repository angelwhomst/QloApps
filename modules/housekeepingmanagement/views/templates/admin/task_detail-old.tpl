...
.hk-back-btn { margin-bottom: 16px; display: inline-block; background: #f0f0f0; color: #333; border: none; border-radius: 6px; padding: 8px 16px; text-decoration: none; font-size: 14px; }
.hk-back-btn:hover { background: #e0e0e0; }
...
<a href="{$back_link|escape:'html'}" class="hk-back-btn">← Back to Dashboard</a>
  ...
  document.querySelectorAll('.hk-detail-toggle-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      fetch(window.location.href, {
        method: 'POST',
        headers: headers,
        body: 'ajax=1&action=updateStepStatus&id_task_step=' + encodeURIComponent(id_task_step) + '&status=' + encodeURIComponent(status) + '&id_task={$task.id_task|intval}&token={$current_token|escape:'url'}'
      })
      .then(resp => resp.json())
      .then(data => {
        if (data.success) {
  ...
  if (doneBtn) {
    doneBtn.addEventListener('click', function() {
      ...
      fetch(window.location.href, {
        method: 'POST',
        headers: headers,
        body: 'ajax=1&action=markTaskDone&id_task={$task.id_task|intval}&token={$current_token|escape:'url'}'
      })
      .then(resp => resp.json())
      .then(data => {
        if (data.success) {
